#!/usr/bin/env node

// Real GLM response from user
const glmResponse = {
  "choices": [{
    "finish_reason": "tool_calls",
    "index": 0,
    "message": {
      "content": "\n",
      "reasoning_content": "用户询问上海今天的天气情况。",
      "role": "assistant",
      "tool_calls": [{
        "function": {
          "arguments": "{\"city\":\"上海\"}",
          "name": "getWeather"
        },
        "id": "call_-8003854072796443857",
        "index": 0,
        "type": "function"
      }]
    }
  }],
  "created": 1767617584,
  "id": "20260105205302f2775c439f484ac6",
  "model": "glm-4.5",
  "object": "chat.completion",
  "usage": {
    "completion_tokens": 56,
    "prompt_tokens": 221,
    "prompt_tokens_details": {
      "cached_tokens": 215
    },
    "total_tokens": 277
  }
};

// normalizeToCamelCase simulation
function snakeToCamel(str) {
  return str.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
}

function isSnakeCase(str) {
  return /_/.test(str) && !/[A-Z]/.test(str);
}

function normalizeToCamelCase(obj, deep = true) {
  if (obj === null || obj === undefined) return obj;
  if (Array.isArray(obj)) {
    return obj.map((item) => normalizeToCamelCase(item, deep));
  }
  if (typeof obj !== 'object') return obj;

  const normalized = {};
  for (const [key, value] of Object.entries(obj)) {
    const normalizedKey = isSnakeCase(key) ? snakeToCamel(key) : key;
    if (deep && typeof value === 'object' && value !== null && !Array.isArray(value)) {
      normalized[normalizedKey] = normalizeToCamelCase(value, deep);
    } else if (deep && Array.isArray(value)) {
      normalized[normalizedKey] = value.map(item =>
        typeof item === 'object' && item !== null
          ? normalizeToCamelCase(item, deep)
          : item
      );
    } else {
      normalized[normalizedKey] = value;
    }
  }
  return normalized;
}

console.log('=== Original ===');
console.log('choices[0].finish_reason:', glmResponse.choices[0].finish_reason);
console.log('choices[0].message.tool_calls exists:', !!glmResponse.choices[0].message.tool_calls);
console.log('tool_calls:', JSON.stringify(glmResponse.choices[0].message.tool_calls, null, 2));

const normalized = normalizeToCamelCase(glmResponse, true);
console.log('\n=== After normalizeToCamelCase ===');
console.log('choices[0].finishReason:', normalized.choices[0].finishReason);
console.log('choices[0].message.toolCalls exists:', !!normalized.choices[0].message.toolCalls);
console.log('toolCalls:', JSON.stringify(normalized.choices[0].message.toolCalls, null, 2));
console.log('reasoningContent:', normalized.choices[0].message.reasoningContent);
console.log('promptTokensDetails:', normalized.choices[0].promptTokensDetails || normalized.usage?.promptTokensDetails);

console.log('\n=== Extraction Test ===');
const extracted = normalized.choices[0].message.toolCalls;
console.log('Can extract toolCalls?', extracted ? 'YES ✅' : 'NO ❌');
console.log('Array length:', extracted?.length);
console.log('First tool:', extracted?.[0]?.function?.name);
