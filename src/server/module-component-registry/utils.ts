...
export function extractImports(code: string): ImportDeclaration[] {
  ...
  try {
    ...
    console.error('[component-registry] ❌ 解析导入失败:', error);
  }
 ...
 * 从组件信息数组构建文件 hash 记录
 *
 * @param fileHashes - 文件路径到 hash 的映射
 * @param components - 组件信息数组
 * @returns 文件 hash 记录数组
 */
export function buildFileHashRecords(
 ...
 * 找出受组件变更影响的组件集合
 *
 * @param changedComponents - 变更的组件名数组
 * @param components - 组件信息映射
 * @returns 受影响的组件名数组（包含变更的组件和所有依赖者）
 */
export function findAffectedComponents(
  changedComponents: string[],
  components: Record<string, ComponentInfo>
): string[] {
  const affected = new Set<string>(changedComponents);

  // 使用 BFS 找出所有依赖者
  const queue = [...changedComponents];
  const visited = new Set(changedComponents);
